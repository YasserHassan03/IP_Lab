...
void Display_hex(char* text){
	...
	for (int i =0; i<6;i++){
		if (text[i]=='1'){
			if (i==0){
				IOWR_ALTERA_AVALON_PIO_DATA(HEX_0_BASE,0b0000011);
			}
			else if (i==1){
				IOWR_ALTERA_AVALON_PIO_DATA(HEX_1_BASE,0b0000011);
			}
			else if (i==2){
				IOWR_ALTERA_AVALON_PIO_DATA(HEX_2_BASE,0b0000011);
			}
			else if (i==3){
				IOWR_ALTERA_AVALON_PIO_DATA(HEX_3_BASE,0b0000011);
			}
			else if (i==4){
				IOWR_ALTERA_AVALON_PIO_DATA(HEX_4_BASE,0b0000011);
			}
			else if (i==5){
				IOWR_ALTERA_AVALON_PIO_DATA(HEX_5_BASE,0b0000011);
			}
		}
		else if(text[i]=='2'){
			if (i==0){
...
//Lab code for reading string undertsand this and adapt it 
...
int read_chars() {
	...
	while (running) {									// Keep running until QUITLETTER is encountered
		...
		Display_hex(text);						// Print input text
	}
