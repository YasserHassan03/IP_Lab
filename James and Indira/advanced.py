...
def send_on_jtag():
    ...
    # subprocess allows python to run a bash command*       
        while True: #possibly print a special key from fpga if button is pressed to start/stop loop
            ...
            try:
                #vals = vals.decode("utf-8")                                     # turn the byte-chars into a string                          # split the string according to the defined delimiters <-->
                file.write(vals.decode("utf-8")  )   
                client_socket.send(vals)                                  # return the data within the delimtiers <-->
