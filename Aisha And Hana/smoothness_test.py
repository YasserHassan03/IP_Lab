...
def smoothness_score(x_vals, y_vals, z_vals, time_interval):
    ...
    for i in range(1, len(y_vals)):
        if y_vals[i] != []:
            ...
            for i in range (len(y_jerk_magnitudes)):
                start = max(0,i-window_size)
                end = min(len(y_jerk_magnitudes),i+window_size)
    ...
    for i in range(1, len(z_vals)):
        if z_vals[i] != []:
            ...
            for i in range (len(z_jerk_magnitudes)):
                start = max(0,i-window_size)
                end = min(len(z_jerk_magnitudes),i+window_size)
